...
pub async fn run_clean(all: bool, purge: bool) -> CanonResult<()> {
    ...
    if purge {
        ...
        if removed_items.is_empty() {
            ...
            println!();
            println!("{}", style("Canon has been completely removed from this project").yellow());
        }
        ...
        if canon_dir.exists() {
            ...
            println!("Run {} to re-download dependencies", style("canon install").cyan());
        } else {
        ...
        println!("{} cached specifications...", style("Cleaning").cyan().bold());
